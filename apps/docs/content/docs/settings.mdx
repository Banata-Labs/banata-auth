...
A read-only identifier for your project. This is derived from the `appName` in your `BanataAuthConfig` and is used by SDKs to identify which Banata Auth instance to connect to. You can copy it to your clipboard with one click.
...
> **Note:** The `clientId` field is read-only and derived from the server's `appName` configuration. It cannot be changed via the API.
