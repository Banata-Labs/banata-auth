...
For most customer apps, start with [Hosted Sign-In](/docs/hosted-ui) and `@banata-auth/nextjs`.

Use `@banata-auth/react` when you intentionally want embedded auth UI inside your own React app or you need Banata auth state/hooks inside client components.
