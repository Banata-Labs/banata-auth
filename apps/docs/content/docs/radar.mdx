...
The default route set protects sign-in, sign-up, password reset, magic link, email OTP, phone OTP, and linked-device authorization requests. Add custom `protectedPaths` only when your app exposes additional sensitive auth mutations.
...
Production operators can inspect and clear live limiter buckets through the SDK:

```ts
const { buckets } = await banata.configuration.listRateLimitBuckets({
  projectId: "project_123",
  limit: 100,
});

await banata.configuration.resetRateLimitBucket({
  projectId: "project_123",
  key: buckets[0]?.key,
});
```

Use bucket resets for support cases where a legitimate user is locked out after repeated failed attempts. Clearing a whole project by omitting `key` should be treated as an incident action because it removes active brute-force protection for that project.
