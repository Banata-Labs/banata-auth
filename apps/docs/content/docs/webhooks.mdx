...
Operators can inspect delivery records and create replay records for idempotent reprocessing:

```ts
await banata.webhooks.listDeliveries({
  endpointId: "wh_123",
  status: "failed",
  limit: 50,
});

await banata.webhooks.replayDelivery({
  deliveryId: "whd_123",
});
```

Replay creates a new pending delivery with `replayOfDeliveryId` pointing at the original delivery. Only replay events that your receiver can deduplicate safely.
