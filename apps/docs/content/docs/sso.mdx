...
Routing only becomes production-ready after the domain is verified. New or updated connections start with `domainVerified: false`; the domain verification flow marks the connection verified only when every routing domain has a matching verified DNS record.
...
Before enabling an enterprise connection for production routing, validate its required fields and domain status:

```typescript
const report = await client.sso.validateConnection("sso_xxx");
if (report.status === "failed") {
  console.log(report.issues);
}
```

Validation checks routing domains, domain verification, OIDC issuer/client settings, discovery or JWKS availability, SAML metadata, signed assertions, and signed AuthnRequest posture. A connection with an unverified routing domain returns a `domain_not_verified` error and should not be used for production routing.
...
When you associate domains with an SSO connection, Banata Auth enforces that only users with matching verified email domains are routed through that connection. This prevents unauthorized domain claims and ensures users are directed to the correct IdP.
...
- [Organizations](/docs/organizations-overview) -- Learn how to create and manage the organizations that SSO connections belong to.
- [Domains](/docs/domains) -- Verify the email domains used for SSO routing.
