...
Hosted sign-in uses the same model:

- `client_id` selects the project for public hosted-auth lookup
- the server-side API key is the hard scope boundary for customer app traffic
