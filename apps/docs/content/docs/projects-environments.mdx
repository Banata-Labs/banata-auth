...
2. A `super_admin` role seeded with all built-in permissions (23 permissions).
...
3. **Scope injection**: The dashboard automatically injects `projectId` into API calls. Server-side integrations using the SDK should ensure they are calling the correct project's endpoints.
