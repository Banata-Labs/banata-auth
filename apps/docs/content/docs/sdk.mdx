...
| `banata.phoneAndDevices` | Start phone OTP flows and manage linked devices |
...
const githubReadiness = await banata.configuration.validateSocialProviderSetup("github");

if (!githubReadiness.ready) {
  console.log(githubReadiness.errors);
}
```
...
Use `validateSocialProviderSetup(...)` before production launch to confirm credentials are present, demo credentials are not in use, and the HTTPS callback URLs that must be registered in the OAuth provider console are known. This local validation does not replace the required real GitHub/Google callback test.

---

## Phone and Devices

Use `banata.phoneAndDevices` from trusted backend code to start Phone OTP flows, approve linked devices, revoke devices, and issue POS offline snapshots.
...
await banata.phoneAndDevices.startDeviceAuthorization({
  projectId: "proj_123",
...
For the full flow, including mobile primary login, linked-device approval, revocation, and offline POS snapshots, see [Phone & Devices](/docs/phone-and-linked-devices).
...
// After deploying and verifying the replacement:
await banata.apiKeys.delete("key_old");
```

`rotate(...)` creates a replacement key. It leaves the old key active by default so production traffic can be moved safely before revocation.
