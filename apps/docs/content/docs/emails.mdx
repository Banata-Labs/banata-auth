...
Banata supports six email delivery providers out of the box:
...
- **Cloudflare Email Service** — [Cloudflare REST API](https://developers.cloudflare.com/email-service/api/send-emails/rest-api/)

Cloudflare Email Service is the outbound email product. Email Routing alone only handles inbound routing and forwarding, so use Email Service when Banata needs to send transactional email.

### Setting Up a Provider

1. Go to **Emails > Providers** in the dashboard.
2. Click **Enable** on your preferred provider.
3. Enter the required credentials in the fields that appear.
4. Click **Save Key**.
...
Before production launch, validate the active provider from **Emails > Providers** or through the configuration API. Validation checks that the selected provider is supported, enabled, and has the credentials Banata needs for that provider.

Validation does not replace a real send. After the setup report is ready, send a test email through the active provider and confirm delivery in the provider logs.
...
// Switch to Cloudflare Email Service
await saveEmailProviderConfig({
  providers: {
    cloudflare: {
      enabled: true,
...
const readiness = await validateEmailProviderConfig("cloudflare");
