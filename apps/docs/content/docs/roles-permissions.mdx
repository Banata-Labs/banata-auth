...
For multi-app projects, use the `app.product_surface.resource.action` format so permissions cannot collide across products. Examples:

- `ecommerce.pos.terminal.approve`
- `ecommerce.admin.refund.create`
- `whatspoppin.mobile.linked_device.approve`
- `automation.whatsapp.instance.delete`

The shared production readiness contracts expose `permissionNamespaceSchema` and `validatePermissionNamespaceCatalog` for teams that want to validate this catalog before launch.
...
4. Enter a slug using the `resource.action` format, or `app.product_surface.resource.action` for multi-app projects.
...
3. **Keep permissions granular.** Stick to `resource.action` for a single app, or `app.product_surface.resource.action` when one project serves multiple apps. Fine-grained permissions like `invoice.approve` and `invoice.read` give you more flexibility than a broad `invoice.manage`.
