...
4. Server calls your sendOtp callback to deliver the code via email
...
> **Note:** Like magic links, the OTP callback receives `email` (a string) rather than `user` (an object), because the user may not exist yet -- email OTP can create new accounts on first use.
...
If you provide a custom `sendOtp` callback in your config, it takes precedence over the built-in template.
...
1. Check your `sendOtp` callback for errors
2. Check the spam/junk folder
3. Verify the email provider API key is correct
4. In development, log the OTP to the console instead of sending an email
