...
To also verify project-scoped public config, include a server API key, project client ID, or project ID:

```bash
BANATA_API_KEY=project_server_api_key bun run verify:auth-live-smoke
...
- `auth-security-headers`, `hosted-ui-security-headers`, and `docs-security-headers` confirm HSTS, CSP, frame, MIME sniffing, referrer, permissions, COOP, and CORP headers are present. The auth and hosted UI checks also require `Cache-Control: no-store`.
